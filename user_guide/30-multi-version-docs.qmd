...
3. **Assemble**: Merge the per-version `_site/` outputs into the final site: latest version at the root, historical versions under `/v/<version>/` with a leading `v` removed from the tag. Generate the version map, aliases, and redirect files.
...
- Store the latest Quarto project in `docs/_quarto/default/`, regardless of its tag. Great Docs recreates it at the start of each build.
- Store each historical Quarto project in `docs/_quarto/<tag>/`, retaining its own `_site/` output. Great Docs recreates only marked projects selected for the build.
- Deploy the assembled site from `docs/_site/`, with the latest version at its root and historical versions under `v/<version>/`.
- Cache Git-introspected API snapshots in `docs/.cache/`, or beside a custom configuration file.

Every Quarto project sits two levels below the configuration directory, whether or not you configure `versions:`. Non-versioned builds also use `docs/_quarto/default/`.

A latest tag of `2.0.0` still uses `docs/_quarto/default/`, not `docs/_quarto/2.0.0/`.
Historical tag `v1.5.0` uses `docs/_quarto/v1.5.0/`, but publishes at
`docs/_site/v/1.5.0/`, never `v/v1.5.0/`. Tag `1.5.0` has the same published URL.
Selectors and Git lookups retain the configured tag, including its `v` prefix.

For a custom layout, run `great-docs build --config website/great-docs.yml`.
The same structure then lives under `website/`.
...
Great Docs deletes only Quarto project directories whose `_quarto.yml` contains its generated marker. It preserves unmarked directories. If one would receive a configured version, the build stops and asks you to resolve the conflict.
