...
def test_lint_includes_shared_sources(tmp_path: Path, directory: str, check: str) -> None:
    ...
    if check == "ambiguous-xref":
        _check_ambiguous_references(
            ...
            _gather_prose([], tmp_path, layout),
            result,
        )
        expected.add("README.md")
    else:
        _check_stale_versions(tmp_path, result, layout)
